...
if (!function_exists('mo_get_portfolio_categories_filter')) {
    ...
    function mo_get_portfolio_categories_filter() {
        ...
        if (!empty($portfolio_categories)) {
            ...
            $output .= '<li class="filter-text">' . __('[:en]Portfolio:[:zh]範例[:]', 'mo_theme') . '</li>';

            $output .= '<li class="segment-0"><a class="portfolio-filter" data-value="*" href="#">' . __('[:en]All[:zh]全部[:]', 'mo_theme') . '</a></li>';
